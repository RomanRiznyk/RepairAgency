...
public class ProcessRegistrationCommand implements Command {
...
//    private static final String ALREADY_EXIST = "Such login is already exist, try another one or login with it.";
   ...
   // private static final String SHOULD_BE_EQUALS = "Passwords should be equals, try again.";
    ...
    public ProcessRegistrationCommand(UserService userService) {
    ...
    public String execute(HttpServletRequest request, HttpServletResponse response){
        ...
        if( login == null || login.equals("") || password == null || password.isEmpty()){ //todo what's better isEmpty or equals("")
            Util.processInvalidCredentials(request, SHOULD_BE_NOT_NULL);  // todo extract into common method with Login
            Util.loggingEnds(this, logger);
            return PathConstants.REGISTER_PAGE;
        }

        if(!login.matches("^[A-z\\p{IsCyrillic}0-9._-]{3,}$")){
            Util.processInvalidCredentials(request, SHOULD_BE_VALID_LOGIN);
            Util.loggingEnds(this, logger);
            return PathConstants.REGISTER_PAGE;
        }

        if(!password.matches("^[a-zA-Z0-9._-]{3,}$")){
            Util.processInvalidCredentials(request, SHOULD_BE_VALID_PASSWORD);
            Util.loggingEnds(this, logger);
            return PathConstants.REGISTER_PAGE;
        }

        if (!email.matches("^(.+)@(.+)\\.(.+)$") &  !(email == null || "".equals(email))) {
            Util.processInvalidCredentials(request, SHOULD_BE_VALID_EMAIL);
            Util.loggingEnds(this, logger);
            return PathConstants.REGISTER_PAGE;
        }

        if (!password.equals(confirmPassword)) {
            Util.processInvalidCredentials(request, SHOULD_BE_EQUALS);  // todo extract into common method with Login
            Util.loggingEnds(this, logger);
            return PathConstants.REGISTER_PAGE;
        }
        ...
        if (!newUser.isPresent()) {
            Util.processInvalidCredentials(request, "\"" +login + "\"" + ALREADY_EXIST);
            Util.loggingEnds(this, logger);
            return PathConstants.REGISTER_PAGE;
        }

        System.out.println("NEW USER LOGIN = " + newUser.get().getLogin());
        Util.addUserIntoSessionAndContext(request, logger, newUser.get());
