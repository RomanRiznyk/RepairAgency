...
public interface SqlConstants { //todo what type of class it should be?
    ...
    // --------------- USER SQL STATEMENTS --------------//
    String SELECT_INVOICES_BY_EMAIL = "SELECT * FROM receipt WHERE user_id IN (SELECT id FROM user WHERE email=?)";
    ...
    String INSERT_USER = "INSERT INTO user (login, password) VALUES (?, ?)";
    String INSERT_USER_WITH_MAIL = "INSERT INTO user (login, password, email) VALUES (?, ?, ?)";
